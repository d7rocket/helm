  ...
  return new Promise((resolve) => {
    eachLine(file, (o) => {
      ...
      if (o.type === 'user' && o.message) {
        ...
        // slash-command invocations arrive as <command-name> markup
        if (typeof c === 'string') {
          const cmd = c.match(/<command-name>\/?([\w:.-]+)<\/command-name>/);
          if (cmd) sum.skills[cmd[1]] = (sum.skills[cmd[1]] || 0) + 1;
        }
        ...
        if (Array.isArray(m.content)) {
          for (const item of m.content) {
            if (item.type === 'tool_use') {
              ...
              if (item.name === 'Skill' && item.input && item.input.skill) {
                const sk = String(item.input.skill);
                sum.skills[sk] = (sum.skills[sk] || 0) + 1;
              }
            }
          }
        }
...
// Claude Code slugifies a cwd by replacing non-alphanumerics with '-' (one per
// character, so "C:\Users\X" → "C--Users-X"). Try that form first, then the
// collapsed-runs form, so either flavor strips back off.
const HOME_SLUGS = [
  os.homedir().replace(/[^A-Za-z0-9]/g, '-').replace(/-+$/, ''),
  os.homedir().replace(/[^A-Za-z0-9]+/g, '-').replace(/-+$/, ''),
];
...
function projectNameFromCwd(cwd, slug) {
  ...
  let s = slug;
  for (const hs of HOME_SLUGS) {
    if (hs && s.startsWith(hs)) { s = s.slice(hs.length).replace(/^-+/, ''); break; }
  }
...
// Aggregate skill/command invocation counts across every session.
async function getSkillStats() {
  const sessions = await getSessions();
  const stats = {};
  for (const s of sessions) {
    for (const [name, n] of Object.entries(s.skills || {})) {
      const st = (stats[name] = stats[name] || { count: 0, last: null });
      st.count += n;
      if (s.end && (!st.last || s.end > st.last)) st.last = s.end;
    }
  }
  return stats;
}
...
// ---------------------------------------------------------------------------
// Config surface — the small whitelist of Claude Code files HELM may show/edit.
// Reads and writes only ever touch paths re-enumerated by this function, so a
// crafted request can never reach outside it. `.env*` never appears here.
// ---------------------------------------------------------------------------

const HOOKS_DIR = path.join(CLAUDE_DIR, 'hooks');
const DATA_DIR = path.join(__dirname, '..', 'data');
const EDIT_BACKUPS = path.join(DATA_DIR, 'edit-backups.json');
const CONTENT_MAX = 512 * 1024;

async function listConfigFiles() {
  const mk = (file, name, editable) => {
    try {
      const st = fs.statSync(file);
      if (!st.isFile()) return null;
      return { path: file, name, editable, sizeKB: Math.round(st.size / 102.4) / 10 };
    } catch { return null; }
  };

  const globalFiles = [
    mk(path.join(CLAUDE_DIR, 'CLAUDE.md'), 'CLAUDE.md — global instructions', true),
    mk(path.join(CLAUDE_DIR, 'settings.json'), 'settings.json', false),
    mk(path.join(CLAUDE_DIR, 'keybindings.json'), 'keybindings.json', false),
  ].filter(Boolean);

  const memory = [];
  try {
    for (const slug of fs.readdirSync(PROJECTS_DIR)) {
      const mdir = path.join(PROJECTS_DIR, slug, 'memory');
      let files = [];
      try { files = fs.readdirSync(mdir); } catch { continue; }
      const proj = projectNameFromCwd(null, slug);
      for (const f of files.filter(x => x.endsWith('.md')).sort()) {
        const m = mk(path.join(mdir, f), `${proj} / ${f}`, true);
        if (m) memory.push(m);
      }
    }
  } catch {}
  ...
  for (const p of await getProjects()) {
    for (const cand of [path.join(p.path, 'CLAUDE.md'), path.join(p.path, '.claude', 'CLAUDE.md')]) {
      if (seen.has(cand)) continue;
      seen.add(cand);
      const m = mk(cand, `${p.name} / ${path.relative(p.path, cand)}`, true);
  ...
  const hooks = [];
  try {
    for (const f of fs.readdirSync(HOOKS_DIR).sort()) {
      const m = mk(path.join(HOOKS_DIR, f), f, false);
      if (m) hooks.push(m);
    }
  } catch {}

  return [
    { title: 'GLOBAL', files: globalFiles },
    { title: 'MEMORY', files: memory },
    { title: 'PROJECT CLAUDE.MD', files: projects },
    { title: 'HOOKS (READ-ONLY)', files: hooks },
  ].filter(g => g.files.length);
}

async function findConfigFile(p) {
  for (const g of await listConfigFiles()) {
...
async function readConfigFile(p) {
  const f = await findConfigFile(p);
  if (!f) return { error: 'not on the editable whitelist' };
  try {
    const body = fs.readFileSync(f.path, 'utf8');
    return { path: f.path, name: f.name, editable: f.editable, content: body.slice(0, CONTENT_MAX), truncated: body.length > CONTENT_MAX };
  } catch (e) { return { error: e.message }; }
}

// Keep the last 30 pre-edit versions so a bad save is always recoverable.
function backupBeforeWrite(p) {
  let backups = [];
  try { backups = JSON.parse(fs.readFileSync(EDIT_BACKUPS, 'utf8')); } catch {}
  try {
    backups.unshift({ path: p, t: new Date().toISOString(), content: fs.readFileSync(p, 'utf8') });
    backups = backups.slice(0, 30);
    fs.mkdirSync(DATA_DIR, { recursive: true });
    const tmp = EDIT_BACKUPS + '.tmp';
    fs.writeFileSync(tmp, JSON.stringify(backups), 'utf8');
    fs.renameSync(tmp, EDIT_BACKUPS);
  } catch {}
}

async function writeConfigFile(p, content) {
  const f = await findConfigFile(p);
  if (!f) return { error: 'not on the editable whitelist' };
  if (!f.editable) return { error: 'this file is read-only in HELM' };
  if (typeof content !== 'string' || content.length > CONTENT_MAX) return { error: 'content missing or too large' };
  backupBeforeWrite(f.path);
  try {
    // atomic: temp file in the same directory, then rename over the original
    const tmp = f.path + '.helm-tmp';
...
module.exports = {
  ...
  toggleSkill, getSkillSource, getSkillStats, searchTranscripts, getUsage, getRecentlyActive,
