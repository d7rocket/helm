...
async function searchTranscripts(query, limit = 60) {
  ...
  for (const { slug, file } of files) {
    ...
    const snippets = [];
    await eachLine(file, (o) => {
      if (snippets.length >= 3 || o.isSidechain) return;
      ...
      if (o.type === 'user' && o.message && typeof o.message.content === 'string' && !o.isMeta) {
        ...
        for (const it of o.message.content) {
          if (it.type === 'text' && it.text && it.text.toLowerCase().includes(q)) { text = it.text; kind = 'assistant'; break; }
        }
      }
      if (text && text.toLowerCase().includes(q)) {
        ...
        const snip = (start > 0 ? '…' : '') + flat.slice(start, idx + q.length + 90) + '…';
        snippets.push({ t: o.timestamp, kind, text: snip });
      }
    });
    if (snippets.length) {
      const s = byId.get(id) || {};
...
// USD per 1M tokens. ESTIMATE — tune to your actual plan/rates. Cost is derived
// from transcript token counts, so it's an approximation, not a billed figure.
const PRICING = {
  ...
  fable:  { in: 1,  out: 5,  cacheRead: 0.1,  cacheWrite: 1.25 },
...
function estimateSessionCost(s) {
  const modelOut = Object.values(s.models);
  const totalModelOut = modelOut.reduce((a, b) => a + b, 0) || 1;
  let cost = 0;
  const entries = Object.entries(s.models);
  if (!entries.length) return 0;
  for (const [m, out] of entries) {
    const p = priceFor(m);
    const share = out / totalModelOut;   // split shared in/cache tokens by output share
    cost += (out / 1e6) * p.out;
    cost += (s.tokens.in * share / 1e6) * p.in;
    cost += (s.tokens.cacheRead * share / 1e6) * p.cacheRead;
    cost += (s.tokens.cacheWrite * share / 1e6) * p.cacheWrite;
  }
  return cost;
}
...
async function getUsage(days = 30) {
  ...
  for (const s of sessions) {
    ...
    const c = estimateSessionCost(s); totals.cost += c;
    for (const [d, v] of Object.entries(s.perDay)) {
      (daily[d] = daily[d] || { out: 0, prompts: 0 });
      daily[d].out += v.out; daily[d].prompts += v.prompts;
    }
  ...
  for (let i = days - 1; i >= 0; i--) {
    const dt = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i);
    const ds = dayOf(dt);
    series.push({ date: ds, out: (daily[ds] || {}).out || 0, prompts: (daily[ds] || {}).prompts || 0 });
  }

  const models = Object.entries(byModel).map(([m, out]) => ({ model: m, out })).sort((a, b) => b.out - a.out);
  const projects = Object.entries(byProject)
    .map(([project, v]) => ({ project, ...v })).sort((a, b) => b.out - a.out).slice(0, 20);

  return { totals, series, models, projects, days };
}
